...
/**
 * Prefer DeepSeek V4 Flash for extraction when available, otherwise fallback to the current model.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api>> {
	const deepseekModel = modelRegistry.find("deepseek", "deepseek-v4-flash");
	if (deepseekModel) {
		const auth = await modelRegistry.getApiKeyAndHeaders(deepseekModel);
		if (auth.ok) {
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Select the best model for extraction (prefer DeepSeek V4 Flash, fallback to current)
